...
# set file path
A = 'A' + '{0:03}'.format(38)  # Action class label
...
show_mode = 1   # 0: show all data, 1: show only good, 2:show only bad
save_exception_list = False     # save exception file list
with_append = True             # save by appending to existing file
exception_avi_list = []
exception_skel_list = []
...
if save_exception_list:
    if with_append:
        ...
        with open(A+'_exeception_avi_list.txt', 'r') as f_avi:
            load_exception_avi_list = f_avi.readlines()

        with open(A+'_exeception_skeleton_list.txt', 'r') as f_skel:
            load_exception_skel_list = f_skel.readlines()
